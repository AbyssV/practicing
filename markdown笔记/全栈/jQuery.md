...
事件冒泡：当一个元素接收到事件的时候 会把他接收到的事件传给自己的父级，一直到window（注意这里传递的仅仅是事件 并不传递所绑定的事件函数。所以如果父级没有绑定事件函数，就算传递了事件 也不会有什么表现 但事件确实传递了）点击事件跟页面显示出来的位置是没关系的，而是跟html代码中的位置有关系
...
> 在 jQuery 中，`on()`方法是用来绑定事件处理程序的。它不仅可以绑定事件到当前选定的元素，还可以用来设置事件委派。
>
> 事件委派是一种事件处理模式，在这种模式中，不是直接将事件处理程序绑定到某个特定的元素，而是绑定到一个父元素或者更高级别的祖先元素。当事件被触发时，事件会在文档树上冒泡，直到它到达绑定了事件处理程序的那个元素。在这个过程中，每当事件到达一个元素，jQuery 都会检查该元素是否匹配我们指定的选择器。如果匹配，那么就会执行我们定义的事件处理程序。
>
> 事件委派的优势在于，即使在事件处理程序被绑定之后，动态添加到文档中的元素仍然可以响应事件。这是因为它们的事件会冒泡到绑定了事件处理程序的祖先元素，然后再由 jQuery 判断它们是否匹配我们指定的选择器。这是直接绑定事件处理程序无法做到的，因为直接绑定的事件处理程序只能在绑定时作用于那些已经存在的元素。
>
> 例如，假设我们有一个列表，并且我们希望在点击每个列表项时都有反应。我们还希望在点击新添加的列表项时也有反应。那么我们可以这样做：
>
> ```
> javascriptCopy code
> $('ul').on('click', 'li', function() {
...
> ```
>
> 在这个例子中，事件处理程序被绑定到 `ul` 元素，然后委派到 `li` 元素。当 `li` 元素被点击时，事件会冒泡到 `ul` 元素，然后 jQuery 会检查该 `li` 元素是否真的被点击。如果是，那么就会执行我们的事件处理程序。
>
> 在 jQuery 中使用事件委派时，只有满足选择器条件的元素才会触发事件处理程序，而非直接的父元素或绑定事件的元素。
>
> 所以，父元素（在这个例子中是 `ul` 元素）不会执行这个事件处理程序，除非它也满足了选择器的条件。
