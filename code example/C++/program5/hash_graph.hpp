...
namespace ics {
  ...
  private:
    ...
    //HashGraph<T> instance variables
    NodeMap node_values;
    EdgeMap edge_values;
  };
...
//Copy all nodes and edges from g
  ...
  HashGraph<T>::HashGraph (const HashGraph& g) {
    for (const NodeMapEntry& x : g.node_values)
      add_node(x.first);

    for (const EdgeMapEntry& x : g.edge_values)
...
//  throw a GraphError exception with appropriate descriptive text
  ...
  T HashGraph<T>::edge_value(NodeName origin, NodeName destination) const {
    Edge p = Edge(origin,destination);
    if (!edge_values.has_key(p)) {
      std::ostringstream exc;
      exc << "HashGraph.edge_value: " << p << " not an edge in the graph";
      throw GraphError(exc.str());
    }

    return edge_values[p];
  }
...
//  throw a GraphError exception with appropriate descriptive text
  ...
  int HashGraph<T>::in_degree(NodeName node_name) const {
    if (!node_values.has_key(node_name)) {
      std::ostringstream exc;
      exc << "HashGraph.in_degree: " << node_name << " not node in the graph";
      throw GraphError(exc.str());
    }
...
//  throw a GraphError exception with appropriate descriptive text
  ...
  int HashGraph<T>::out_degree(NodeName node_name) const {
    if (!node_values.has_key(node_name)) {
      std::ostringstream exc;
      exc << "HashGraph.out_degree: " << node_name << " not node in the graph";
      throw GraphError(exc.str());
    }
...
//  throw a GraphError exception with appropriate descriptive text.
  ...
  int HashGraph<T>::degree(NodeName node_name) const {
    if (!node_values.has_key(node_name)) {
      std::ostringstream exc;
      exc << "HashGraph.degree: " << node_name << " not node in the graph";
      throw GraphError(exc.str());
    }
...
//  appropriate  descriptive text
  ...
  auto HashGraph<T>::out_nodes(NodeName node_name) const -> const NodeSet& {
    if (!node_values.has_key(node_name)) {
      std::ostringstream exc;
      exc << "HashGraph.out_nodes: " << node_name << " not node in the graph";
      throw GraphError(exc.str());
    }
...
//  appropriate descriptive text
  ...
  auto HashGraph<T>::in_nodes(NodeName node_name) const -> const NodeSet& {
    if (!node_values.has_key(node_name)) {
      std::ostringstream exc;
      exc << "HashGraph.in_nodes: " << node_name << " not node in the graph";
      throw GraphError(exc.str());
    }
...
//  appropriate descriptive text
  ...
  auto HashGraph<T>::out_edges(NodeName node_name) const -> const EdgeSet& {
    if (!node_values.has_key(node_name)) {
      std::ostringstream exc;
      exc << "HashGraph.out_edges: " << node_name << " not node in the graph";
      throw GraphError(exc.str());
    }
...
//  appropriate descriptive text
  ...
  auto HashGraph<T>::in_edges(NodeName node_name) const -> const EdgeSet& {
    if (!node_values.has_key(node_name)) {
      std::ostringstream exc;
      exc << "HashGraph.in_edges: " << node_name << " not node in the graph";
      throw GraphError(exc.str());
    }
...
//Add these node names and update edge_values and the LocalInfos of each node
  ...
  void HashGraph<T>::add_edge (NodeName origin, NodeName destination, T value) {
    ...
    LocalInfo& lo = node_values[origin];
    lo.out_nodes.insert(destination);
    lo.out_edges.insert(Edge(origin,destination));

    LocalInfo& ld = node_values[destination];
    ld.in_nodes.insert(origin);
    ld.in_edges.insert(Edge(origin,destination));
  }
...
// to copy a set and then iterate over it while removing values from the original set
  ...
  void HashGraph<T>::remove_node (NodeName node_name){
    ...
    NodeSet destinations (node_values[node_name].out_nodes);
    for (const std::string& destination : destinations)
      remove_edge(node_name,destination);

    NodeSet origins (node_values[node_name].in_nodes);
    for (const std::string& origin : origins)
      remove_edge(origin,node_name);
...
//Hint: use split and istringstream (the extraction dual of ostreamstring)
  ...
  void HashGraph<T>::load (std::ifstream& in_file, std::string separator) {
    ...
    while (getline(in_file,line)) {
      std::vector<std::string> odv = split(line,separator);
      //Decide the format of the line
      if (odv.size() == 1)
        add_node(line);
      else {
        T v;
        std::istringstream answer(odv[2]);
        answer >> v;
        add_edge(odv[0],odv[1],v);
      }
    }
...
//Hint: this is the easier of the two methods: write and test it first
  ...
  void HashGraph<T>::store(std::ofstream& out_file, std::string separator) {
    for (const NodeMapEntry & n_l : node_values)
      out_file << n_l.first << std::endl;

    for (const EdgeMapEntry& np_e : edge_values)
      out_file << np_e.first.first  << separator
      << np_e.first.second << separator
      << np_e.second       << std::endl;
    out_file.close();
  }
...
//Hint: each copied LocalInfo object should reset from_graph to the this new graph
  ...
  HashGraph<T>& HashGraph<T>::operator = (const HashGraph<T>& rhs){
    ...
    for (NodeMapEntry& x : node_values)  // Not const!
...
//  just check that node names are the same in each
  ...
  bool HashGraph<T>::operator == (const HashGraph<T>& rhs) const{
    //Avoid checking == on LocalInfo (edge_map has this information)
    //So, no checking nodes == rhs.nodes; instead puts node of each in set and check ==
    NodeSet this_nodes;
    NodeSet rhs_nodes;
    for (const NodeMapEntry& n : node_values)
      this_nodes.insert(n.first);
    for (const NodeMapEntry& n : rhs.node_values)
      rhs_nodes.insert(n.first);

    return this_nodes == rhs_nodes && edge_values == rhs.edge_values;
  }
...
//Return whether two graphs are different
  ...
  std::ostream& operator<<(std::ostream& outs, const HashGraph<T>& g) {
    ...
    if (!g.empty()) {
      ...
      for (const typename HashGraph<T>::NodeLocalEntry& x : nodes)
