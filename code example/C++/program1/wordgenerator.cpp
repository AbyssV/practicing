...
#include <time.h>                           //If you commented-out randomize code
...
bool queue_gt(const CorpusEntry& a, const CorpusEntry& b) {
 for (WordQueue::Iterator ac = a.first.begin(), bc= b.first.begin(); ac != a.first.end(); ++ac,++bc)
...
void print_corpus(const Corpus& corpus) {
 ...
 CorpusPQ words_sorted(corpus, queue_gt);
...
int main() {
 try {
   ...
   srand(time(nullptr));  //Uncomment to generate truly random numbers
